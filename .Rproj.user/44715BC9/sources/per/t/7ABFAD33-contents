...
# sample info
sample_info <- readRDS("sample.info.rds")
# sctransform-integrated @data expression values - sparse matrix
exp_mat <- readRDS("data.exp.rds")
all.genes <- sort(rownames(exp_mat))
# Differential expression results
de <- readRDS("data.de.rds")
# Normalized expression from limma
v <- readRDS("data.v.rds")
# AUC TF regulon activity scores from SCENIC
auc <- readRDS("data.auc.rds")
# Differential regulon activity among genotypes (Wilcoxon ranked sum tests)
dra <- readRDS("data.dra.rds")
dra.genes <- sort(unique(unlist(lapply(dra, rownames))))

# cell type color palettes
#cell.names <- levels(meta_tbl$Cell_Type)
#colors_cells <- rainbow(length(cell.names))
#colors_cells <- setNames(colors_cells, cell.names)

colors_cells <- readRDS("colors.cells.rds")
colors_groups <- c(WT.F="#F8766D", WT.M="#00BFC4", SOD1G93A.F="red2", SOD1G93A.M="royalblue")
colors_genotypes <- c(SOD1G93A="red2", WT="royalblue")

# check that the cluster order is the same in the meta data table and the colors vector
#clusters_ordered <- meta_tbl %>% pull(Cell_Type) %>% levels()
#if (!identical(clusters_ordered, names(colors_cells))) stop("unexpected color order")
...
ui <- fluidPage(
  ...
  # layout: title ----
  titlePanel(div(HTML("Single-nucleus RNA-seq of lumbar spinal cord from amyotrophic lateral sclerosis <em>SOD1<sup>G93A</sup></em> model and wild-type mice"))),
      ...
      sidebarLayout(
        ...
        mainPanel(
          fluidRow(
            column(
              width = 5,
              withSpinner(plotOutput("umap_gene_plot"))
            ),

            column(
              width = 5,
              withSpinner(plotOutput("umap_cluster_plot"))
            )
          ),
          ...
          fluidRow(
            column(
              width = 10,
              withSpinner(plotOutput("umap_gene_group_plot", height="800px"))
            )
          ),
